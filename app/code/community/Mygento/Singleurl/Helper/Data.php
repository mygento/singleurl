...
class Mygento_Singleurl_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function getCategories($productId, $storeId, $justIds = false) {
        $collection = Mage::getModel('catalog/category')->getCollection()->addAttributeToSelect('name')->addAttributeToSelect('url_key')->addAttributeToSelect('url_path');
        ...
        $collection->getSelect()->where('cat.product_id = ?', $productId);
